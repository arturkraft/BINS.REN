...
binDates.forEach(binDate => {
    let theDate = binDate.innerHTML
    let utcDate = new Date(theDate)
    let localDate = new Date(utcDate.getTime() + utcDate.getTimezoneOffset() * 60000) //local Date
    let localDateString = localDate.toDateString()
    ...
    else binDate.textContent=formatted_date
})
